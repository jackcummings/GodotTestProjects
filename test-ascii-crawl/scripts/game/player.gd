...
func _init(start_col: int, start_row: int) -> void:
	_col = start_col
	_row = start_row
	visual_pos = Utilities.grid_to_pixel(_col, _row)
	_tween_to = visual_pos
	_tween_from = visual_pos
	
func try_move(direction_col: int, direction_row: int, tile_grid: TileGrid) -> void:
	var new_col = _col + direction_col
	var new_row = _row + direction_row
	if tile_grid.is_walkable(new_col, new_row):
		_col = new_col
		_row = new_row
		# Snap tween_from to current visual pos so chained moves feel responsive
		_tween_from = visual_pos
		_tween_to = Utilities.grid_to_pixel(_col, _row)
		_tween_t = 0.0
		SignalBus.player_moved.emit(_col, _row)

func hp_remove(amount: int) -> void:
	hp -= clampi(hp - amount, 0, max_hp)
	if hp == 0:
		pass

func hp_add(amount: int) -> void:
	hp = clampi(hp + amount, 0, max_hp)
